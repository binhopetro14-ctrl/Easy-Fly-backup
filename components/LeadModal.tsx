...
function DateInput({ value, onChange, className, placeholder }: {
  ...
  return (
    <input
      ...
      placeholder={placeholder || 'DD/MM/AAAA (Ex: 29032026)'}
      maxLength={10}
      className={className}
    />
  );
...
export function LeadModal({ isOpen, onClose, onSave, editingLead, suppliers }: LeadModalProps) {
  ...
  const [formData, setFormData] = useState<Lead>({
    id: '', 
    title: '', name: '', status: 'novo_contato', tags: [], adults: 1, children: 0, babies: 0, luggage23kg: 0,
          ...
          <div className="flex-1 space-y-4">
            <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-gray-100 dark:border-slate-700/50 shadow-sm space-y-5 animate-in slide-in-from-left-4 duration-500">
              <div className="grid grid-cols-2 gap-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input placeholder="Busque ou digite o nome" className="w-full px-4 py-2.5 bg-gray-50 dark:bg-slate-900/50 border border-transparent dark:border-slate-700 rounded-xl outline-none font-bold text-gray-800 dark:text-white focus:border-cyan-400 transition-all shadow-sm" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                </div>
              </div>
                 ...
                 {[
                   ...
                   <div key={c.id} className="flex flex-col items-center gap-2">
                     <div className="flex items-center gap-1.5 min-h-[14px]">
                        ...
                        <span className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">{c.label}</span>
                     </div>
              ...
              <div className="grid grid-cols-3 gap-5">
                 <div className="space-y-1.5 col-span-1">
                   ...
                   <select className="w-full px-4 py-2 bg-gray-50 dark:bg-slate-900 border border-transparent dark:border-slate-700 rounded-xl font-bold text-xs shadow-sm" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as CRMStatus})}>
                 ...
                 <div className="space-y-1.5 col-span-2">
                   ...
                   <div className="flex flex-col gap-2">
                     <div className="flex gap-2">
                       <div className="flex-1 flex gap-1.5 items-center bg-gray-50 dark:bg-slate-900 border border-transparent dark:border-slate-700 rounded-xl px-1 py-1 shadow-sm focus-within:border-cyan-400 transition-all">
                         <input 
                       ...
                       <div className="w-28 flex flex-col gap-1.5">
                          ...
                          <div className="relative">
                            <input 
                              ...
                              className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-900 border border-transparent dark:border-slate-700 rounded-xl font-black text-xs shadow-sm pr-7 text-center focus:border-cyan-400 transition-all font-bold" 
                     ...
                     <div className="flex flex-wrap gap-1.5 min-h-[20px] ml-1">
            ...
            <div className="flex gap-2 p-1 bg-gray-200 dark:bg-slate-800/80 rounded-xl w-fit">
              {['passagem', 'hospedagem', 'seguro', 'carro'].map(t => (
                <button key={t} onClick={() => setActiveItemType(t)} className={`px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeItemType === t ? 'bg-white dark:bg-slate-700 shadow-sm text-cyan-600 dark:text-cyan-400' : 'text-gray-500 hover:text-gray-600'}`}>{t}</button>
              ))}
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 border-t border-gray-100 dark:border-slate-700 pt-6 mt-2 items-end">
                 <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <NumericFormat className="w-full px-5 py-3 bg-white dark:bg-slate-900 border-2 border-gray-100 dark:border-slate-700/50 rounded-2xl font-black text-sm text-gray-800 dark:text-white shadow-sm focus:border-cyan-400 outline-none transition-all" value={currentItem.value} onValueChange={v => setCurrentItem({...currentItem, value: v.floatValue})} thousandSeparator="." decimalSeparator="," />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-gray-400 uppercase ml-1 tracking-widest">Preço Custo (R$)</label>
                      <NumericFormat className="w-full px-5 py-3 bg-gray-50/50 dark:bg-slate-900 border-2 border-transparent dark:border-slate-700/50 rounded-2xl font-black text-sm text-gray-400 dark:text-gray-500 shadow-sm focus:border-gray-200 outline-none transition-all" value={currentItem.cost} onValueChange={v => setCurrentItem({...currentItem, cost: v.floatValue})} thousandSeparator="." decimalSeparator="," />
                    </div>
                 </div>
