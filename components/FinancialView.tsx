          ...
          <div className="bg-white dark:bg-slate-900 px-8 py-4 rounded-3xl border border-gray-100 dark:border-slate-800 shadow-xl flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center bg-slate-50 dark:bg-slate-800/50 p-1 rounded-xl border border-slate-100 dark:border-slate-700/50">
                <button onClick={() => setContasActiveTab('contas')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${contasActiveTab === 'contas' ? 'bg-purple-600 text-white shadow-md' : 'text-slate-400 hover:text-purple-600'}`}>Contas</button>
                <button onClick={() => setContasActiveTab('config')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${contasActiveTab === 'config' ? 'bg-purple-600 text-white shadow-md' : 'text-slate-400 hover:text-purple-600'}`}>Configurações</button>
              </div>
            </div>
          ...
          {contasActiveTab === 'contas' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div key={account.id} style={{ borderTop: `4px solid ${account.color || '#8b5cf6'}` }} className={`bg-white dark:bg-slate-900 rounded-[24px] shadow-xl border border-gray-100 dark:border-slate-800 overflow-hidden group/card relative flex flex-col transition-all ${account.status === 'Inativo' ? 'opacity-60 grayscale-[0.5]' : ''}`}>
                  <div className="p-5 md:p-6 space-y-4 flex-1">
                    <div className="flex items-start justify-between">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); setMenuOpenId(menuOpenId === account.id ? null : account.id); }} className="p-1.5 text-slate-300 hover:text-purple-500 hover:bg-purple-50 rounded-lg transition-all"><MoreVertical className="w-4 h-4" /></button>
                    </div>
              ...
              <button onClick={() => { setEditingAccount(undefined); setIsAccountModalOpen(true); }} className="bg-white dark:bg-slate-900 rounded-[24px] border-2 border-dashed border-gray-200 dark:border-slate-800 p-6 flex flex-col items-center justify-center gap-3 text-gray-400 hover:text-indigo-600 hover:border-indigo-600 transition-all group">
                <div className="w-10 h-10 rounded-xl bg-gray-50 dark:bg-slate-800 flex items-center justify-center group-hover:bg-indigo-50 transition-all"><Plus className="w-5 h-5" /></div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-[24px] p-5 md:p-6 shadow-xl border border-gray-100 dark:border-slate-800">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2"><label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Receitas (Conta Padrão)</label><select value={tempSettings.defaultIncomeAccountId || ''} onChange={e => setTempSettings({ ...tempSettings, defaultIncomeAccountId: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-purple-300 rounded-xl px-4 py-3 text-[11px] font-bold outline-none transition-all">{accounts.map(acc => <option key={acc.id} value={acc.id}>{acc.name}</option>)}</select></div>
                    <div className="space-y-2"><label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Despesas (Conta Padrão)</label><select value={tempSettings.defaultExpenseAccountId || ''} onChange={e => setTempSettings({ ...tempSettings, defaultExpenseAccountId: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-purple-300 rounded-xl px-4 py-3 text-[11px] font-bold outline-none transition-all">{accounts.map(acc => <option key={acc.id} value={acc.id}>{acc.name}</option>)}</select></div>
                  </div>
                  <div className="flex justify-end pt-2">
                    <button onClick={handleSaveSettings} disabled={isSavingSettings} className="flex items-center gap-2 px-6 py-3 bg-purple-900 hover:bg-purple-950 text-white rounded-xl shadow-xl shadow-purple-500/10 transition-all active:scale-95 text-[10px] font-black uppercase tracking-widest disabled:opacity-50">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-[24px] p-5 md:p-6 shadow-xl border border-gray-100 dark:border-slate-800 flex flex-col gap-6">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="px-6 py-3 bg-purple-900 hover:bg-purple-950 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-purple-500/10 transition-all active:scale-95 flex items-center gap-2"
                  >
                ...
                <div className="grid grid-cols-12 gap-6">
                  <div className="col-span-12 lg:col-span-4 flex flex-col gap-2">
                    <button 
                      ...
                      className={`flex items-center justify-between px-5 py-3 rounded-xl transition-all text-[10px] font-black uppercase tracking-widest ${categoryTypeFilter === 'Despesa' ? 'bg-purple-900 text-white shadow-xl shadow-purple-500/20' : 'bg-slate-50 dark:bg-slate-950 text-slate-400'}`}
                    >
                    ...
                    <button 
                      ...
                      className={`flex items-center justify-between px-5 py-3 rounded-xl transition-all text-[10px] font-black uppercase tracking-widest ${categoryTypeFilter === 'Receita' ? 'bg-purple-900 text-white shadow-xl shadow-purple-500/20' : 'bg-slate-50 dark:bg-slate-950 text-slate-400'}`}
                    >
                        ...
                        {categories
                          ...
                          .map((cat) => {
                            ...
                            return (
                              ...
                              >
                                <div className={`flex flex-col bg-white dark:bg-slate-900 rounded-2xl border transition-all ${isExpanded ? 'border-purple-200 dark:border-purple-800 shadow-md' : 'border-slate-100 dark:border-slate-800'}`}>
                                  <div className="flex items-center justify-between p-3 px-5 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 rounded-2xl cursor-pointer" onClick={() => setExpandedCategoryId(isExpanded ? null : cat.id)}>
                                    ...
                                    <div className="flex items-center gap-2">
                                      <button onClick={(e) => { e.stopPropagation(); setEditingCategory(cat); setIsCategoryModalOpen(true); }} className="p-1.5 text-slate-300 hover:text-purple-500 hover:bg-purple-50 rounded-lg transition-all"><Edit2 className="w-3 h-3" /></button>
                                            ...
                                            {subCats.map(sub => (
                                              <Reorder.Item key={sub.id} value={sub} className="flex items-center justify-between p-2 px-4 hover:bg-white dark:hover:bg-slate-800 rounded-xl group/sub transition-all">
                                                ...
                                                <div className="flex items-center gap-1 opacity-0 group-hover/sub:opacity-100 transition-opacity">
                                                  <button onClick={() => { setEditingCategory(sub); setIsCategoryModalOpen(true); }} className="p-1 text-slate-300 hover:text-purple-500"><Edit2 className="w-2.5 h-2.5" /></button>
